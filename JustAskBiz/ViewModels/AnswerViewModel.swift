...
class AnswerViewModel: ObservableObject{
    ...
    {
        db.collection("questions").document(qDocId).collection("answers").addSnapshotListener{(querySnapshot, Error) in
    ...
    {
        db.collection("questions").document(qDocId).collection("answers")
            .addDocument(data: [
